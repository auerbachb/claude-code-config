...
- **If only P1/P2 (no P0):** STOP — merge-ready. Do NOT trigger `@greptileai`. After the fix-only push, the gate reuses the latest completed trigger-delimited zero-P0 round (issue #1000); do not wait for evidence on the new HEAD.
...
Merge-ready when: no findings (clean), all P1/P2 after fix (no re-review needed; the completed zero-P0 round remains durable after the fix-only push), or P0 fixed + a later triggered re-review is clean. An unanswered latest trigger, a latest completed round containing P0, or complete absence of Greptile review history is not merge-ready. Current-head universal gates still apply.
