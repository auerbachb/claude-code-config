...
| [worktree-status.sh](../worktree-status.sh) | Report one worktree's own HEAD SHA, branch, detached state and root in a single call — the non-refused replacement for a `git -C <wt> …; git -C <wt> …` pair (`../../reference/worktree-isolation-command-shapes.md`); every git call wall-clock bounded |
