...
|--------|---------|
<!-- catalog:rows:begin -->
| [ai-quotas-setup.sh](../ai-quotas-setup.sh) | Register AI subscription accounts (`claude`/`codex`/`cursor`) and their isolated per-account login profiles in `~/.claude/ai-quotas.json`, and report which ones are currently logged in — labels and paths only, never a credential value |
| [ai-quotas.sh](../ai-quotas.sh) | Read every account registered by `/quotas-setup` and print one row per usage window — used %, remaining %, reset time in Eastern, and a countdown — for `claude` (Anthropic OAuth usage endpoint) and `codex` (`codex app-server`, HTTP fallback); display only, never a dispatch or spend gate |
