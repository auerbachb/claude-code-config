...
#         "rate-limited by coderabbit" / "currently rate limited" / "fair usage limits policy" /
#         "next review will be available in"; BugBot usage-limit notices — "couldn't run - usage limit
#         reached" / "hit a usage or spend limit"; "full review triggered", BugBot clean-pass
...
if [[ -n "$SINCE" ]]; then
  NEW_SINCE=$(jq -n \
    ...
    def classify:
      ...
      elif test("rate limit exceeded|rate-limited by coderabbit|currently rate limited|fair usage limits policy|next review will be available in"; "i") then {class: "acknowledgment", reason: "rate limit notice"}
