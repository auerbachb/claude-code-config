...
# The full PR snapshot uses canonical jq programs stored beside this script.
# Resolve them only after the standalone modes above have exited: --help,
# --infer-candidates, and --wait-state-eval deliberately do not depend on these
# full-snapshot filters. A partial deployment is an operational/runtime error,
# which stays on the existing exit-5 contract.
...
CHECK_RUNS_DEDUP="$SCRIPT_DIR/check-runs-dedup.sh"
...
CR_SPLIT=$(echo "$CHECK_RUNS" | jq -f "$PR_STATE_CR_SPLIT_JQ")
...
# (pr-state's canonical classifier, review-substance.sh's substance check,
