      ...
      "background_tasks": [
        {
          ...
          "checkpoint_path": "/tmp/review-checkpoint.json",
          "recovery_path": "/tmp/recover-review",
...
check "terminal task metadata is preserved" jq -e '.background_tasks.items[0] | .task_id == "task-42" and .type == "monitor" and .status == "stopped" and .output_file == "/tmp/review-output.txt" and .checkpoint_path == "/tmp/review-checkpoint.json" and .recovery_path == "/tmp/recover-review"' >/dev/null <<<"$DOC"
