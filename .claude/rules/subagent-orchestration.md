...
Fleet: **Fable 5, Opus 5, Sonnet 5, Haiku 4.5**. Fable 5 has no bare alias and is **never a spawn default** — reserve it for interactive step-ups where a human watches the spend. `/harness-audit`'s monthly cron preserves this by offering a step-up chip instead of spawning unattended. Alias resolution and per-phase rationale: `.claude/agents/README.md` §Model Selection.
