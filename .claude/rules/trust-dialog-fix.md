...
`~/.claude.json` stores three per-project trust/includes flags that reset to `false` on every new project entry, including each new worktree. **All three** must be `true` — setting only `hasTrustDialogAccepted` leaves the includes prompts in place.
