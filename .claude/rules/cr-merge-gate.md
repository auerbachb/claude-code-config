...
**CodeAnt on the CR path:** applies when CodeAnt has a review/comment or check-run on current HEAD. Clean = `APPROVED` on HEAD or completed CodeAnt check with `conclusion: success`; `CHANGES_REQUESTED` blocks only if newer than the latest clean signal on that SHA. A CodeAnt "no PR Review subscription" comment is **not** a block — it appeared on 84% of PRs while CodeAnt approved 360 times (#1199). Threads: Step 1c.
