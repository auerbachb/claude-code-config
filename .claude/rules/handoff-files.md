...
**Write lock (issue #682):** Handoff writes go through `handoff-state.sh`, same `state-lock.sh` lock as #639 — concurrent callers silently lose updates without it. Exit **6** = lock timeout (unchanged, retry).
