...
- **Status (2026-08-12):** **CLOSED** by PR implementing Issue #710. Two hooks installed: `spend-session-tracker.sh` (SessionStart — tags new standalone threads) and `spend-subagent-tracker.sh` (SubagentStop — tags each inline Agent-tool subagent). Append-only TSV log at `~/.claude/spend-telemetry.log`; report via `bash .claude/scripts/spend-telemetry-report.sh`. Full schema, reliability caveats, and AC1 re-run methodology: `.claude/reference/spend-telemetry-pipeline.md`.
