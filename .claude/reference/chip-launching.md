...
- **Execution capability** — can this thread run a pipeline? **Every thread can**, with exactly one exception: a thread under an explicit **capture-mode invariant** (`/issue-maker` Step 2), which is barred from implementation, worktrees, and branches and therefore adopts nothing. There are no other exceptions. "This thread was only for discussing X" is not one, a thread that happened to file some issues is not one, and **the absence of a `## Active Work` table** is not one.
...
- **The capture-mode invariant** — a capture thread cannot adopt, so its work has to leave the thread. This is **session-level, not per-issue**: `/issue-maker` emits **one batch hand-off covering every issue filed in the session** (its Step 9c), never one offer per issue. Per-issue too-big routing then happens in the launched thread through that thread's own `/subagent` Step 4.
...
2. **A genuinely non-executing thread** — capture mode, the one exception above → the single **batch hand-off** is the correct delivery, and it is gated on the repo-wide figure rather than on nothing: offer it while `FREE > 0`, and at `FREE == 0` **defer** it, naming `{ACTIVE}/{CAP}` and the scope (see "Repo-wide active-work cap" below). Offering one chip per issue with nothing counted against it was the 2026-08-18 twenty-thread failure; since [#1191](https://github.com/auerbachb/claude-code-config/issues/1191) every surface can read that figure from `active-work-cap.sh` with no PM context at all, so nothing here is uncounted.
...
**What a batch is, after #1229.** The rule above governs the surfaces that still emit **per-issue** chips — i.e. too-big routing, the one structural chip reason left. A **capture session is no longer such a surface**: it emits a single **batch hand-off** covering every issue it filed (`/issue-maker` Step 9c), so the cap gates that one offer rather than a decrementing per-issue budget — offered while `FREE > 0`, deferred at `FREE == 0` with `{ACTIVE}/{CAP}` and the scope named. One hand-off is also self-limiting in a way N chips were not: the thread it opens runs its issues through `min(pipeline_ceiling, active_work_cap)` itself, whereas N offered chips invited N unbounded clicks.
