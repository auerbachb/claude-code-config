...
|----|-----------|----------------------|------------|---------------------:|-------------|------|:--------:|----------------------------------------|
| **C-01** | Merge-gate verifier (CR 2-clean / BugBot 1-clean / Greptile severity) | `cr-merge-gate.md` Step 1 (authoritative) + reimplemented inline in `merge` Step 2, `wrap` Step 2.1, `continue` Step 8, `phase-c-merger` Step 1, `status` Step 3 | 5 skills + 1 agent | ~2,500 | Fully deterministic given (PR, HEAD SHA, reviewer). Reviewer assignment comes from session-state or live history. | Low — read-only `gh api` queries. Side-effects only on session-state write-back for sticky reviewer. | **P0** | `merge-gate.sh <pr_number> [--reviewer cr\|bugbot\|greptile]` → JSON `{met: bool, reviewer, path, missing: [...], head_sha, ci_status}`. Exits: `0` gate met, `1` gate not met (stdout JSON always), `2` usage error, `3` PR not found / closed, `4` gh/network error. |
| **C-02** | PR state snapshot — threads + checks + 3 comment endpoints + statuses + merge_state + (opt) classify-since | Fully implemented in `.claude/skills/fixpr/audit.sh`; inline shards in `merge`, `wrap`, `continue`, `status`, `pm` (Section 2.1), `phase-b-reviewer` polling loop, `phase-c-merger` Step 1 | 7 skills + 2 agents | ~3,500 (for skills that refetch everything) | Deterministic per-SHA. Classify-since window is deterministic given baseline timestamp. | Low — `fixpr/audit.sh` has been in production; move it to `.claude/scripts/pr-state.sh` and add call-site wrappers in other skills | **P0** | `pr-state.sh [pr_number] [--since <iso-8601>]` → JSON bundle to `/tmp/pr-state-<PR>-<epoch>.json`, prints path on stdout. Exits: `0` OK, `2` usage, `3` not on a branch / no PR, `4` PR closed, `5` gh/network error. (Effectively: promote `fixpr/audit.sh` unchanged, change the output-path prefix, add a thin `--pr N` override so it works outside the current branch.) |
| **C-03** | CR plan comment detection on an issue (substantive filter) | `start-issue` Step 3 (Path A + Path B), `subagent` Step 2, `pm-worker` "Issue Creation" Step 3 | 3 skills | ~800 | Deterministic given issue number + filter regex (exclude "actions performed", require length > 200) | Low — read-only `gh issue view --json comments` + jq filter | **P0** | `cr-plan.sh <issue_number> [--poll <minutes>] [--max-age-minutes N]` → stdout: plan body (plaintext) or empty. Exits: `0` plan found (printed), `1` no plan after poll window, `2` usage, `3` issue not found or closed, `4` gh error. `--poll` enters a 60s polling loop until timeout. |
| **C-04** | All-threads-resolver — fetch unresolved review threads → filter to `coderabbitai\|cursor\|greptile-apps` → `resolveReviewThread` (fallback `minimizeComment`) | `phase-a-fixer` Step 5, `fixpr` Step 4b, `continue` Step 7, rule prose in `cr-github-review.md` "Processing CR Feedback" step 4 | 2 skills + 1 agent + 1 rule | ~1,200 (GraphQL query + mutation loop + fallback is verbose) | Fully deterministic given PR number | Low — only mutating call is the GraphQL resolution itself, which is idempotent | **P0** | `resolve-review-threads.sh <pr_number> [--authors coderabbitai,cursor,greptile-apps] [--dry-run]` → stdout per-thread status lines. Exits: `0` all resolved (or dry-run OK), `1` ≥1 thread could not be resolved (both mutations failed), `2` usage, `3` PR not found, `4` gh error. Dry-run prints the thread IDs that would be resolved and exits 0 without mutating. |
| **C-05** | Root-repo path resolver (`git worktree list | head -1 | awk '{print $1}'`) | `work-log.md` rule (×2), `safety.md` rule, `skill-symlinks.md` rule (×2), `setup-skills-worktree.sh`, `session-start-sync.sh`, `wrap` (×2), `start-issue` (×2), `merge` Step 5b, `pm-worker`, `repair-worktrees.sh`, `pm` Step 2.1 | 3 rules + 5 skills + 2 agents + 2 hooks + 3 existing scripts | ~300 per call site, ~3,500 total prose | Fully deterministic | Near-zero — one-line bash. Extraction risk is over-engineering. | **P0** | `repo-root.sh` → stdout: absolute path of main-worktree root. Exits: `0` OK, `1` not in a git repo / no worktrees found. **Nothing to pipe through jq** — callers use `ROOT_REPO=$(repo-root.sh)` directly. (This is a trivial shell function but extraction standardizes error handling — a detached-HEAD edge case crashes the one-liner silently today.) |
| **C-06** | Cycle-count reconstruction — per-PR review-then-fix pairings | `merge` Step 6, `wrap` Step 2.7, `pm-rate-team` Step 3b, `pm-sprint-review` (same pattern), `pm-data-patterns.md` reference doc | 4 skills + 1 reference | ~900 | Fully deterministic given PR (reviews + commits timeline). | Low — pure read from GitHub API. | **P0** | `cycle-count.sh <pr_number> [--exclude-bots]` → stdout: integer cycle count. Exits: `0` OK (count on stdout), `2` usage, `3` PR not found, `4` gh error. `--exclude-bots` filters reviews whose `user.login` ends in `[bot]` or equals `github-actions` (matches `pm-data-patterns.md` bot filter). |
| **C-07** | GH date-window builder — `SINCE_DATE` + `SINCE_ISO` with colon-offset, ET anchor, macOS + GNU dual-syntax | `pm-rate-team` Step 1, `pm-sprint-review` Step 1, `pm-sprint-plan` Step 1, `pm-team-standup` Step 1, `standup` (embedded in 150-line `date` helper suite), `pm-data-patterns.md` "Time window utilities" | 5 skills + 1 reference | ~400 per call site, ~2,400 total | Fully deterministic | Near-zero — copy the reference-doc snippet into a script | **P0** | `gh-window.sh --days N [--format date\|iso\|both]` → stdout: one value (or two tab-separated for `both`). Exits: `0` OK, `2` usage (missing/invalid `--days`), `3` `date` command failed on this platform. Default `--format both` prints `"$SINCE_DATE\t$SINCE_ISO"`. |

### P1 — Extract next

| ID | Candidate | Where it lives today | Call sites | Tokens/session (est) | Determinism | Risk | Priority | Proposed CLI signature + exit contract |
|----|-----------|----------------------|------------|---------------------:|-------------|------|:--------:|----------------------------------------|
| **C-08** | CI health check — all-check-runs pass / blocking / incomplete splits | `cr-merge-gate.md` Step 1b, `merge` Step 4, `wrap` Step 2.4, `continue` Step 8 fallback paths, `phase-c-merger` Step 2, `cr-polling-commands.md` reference | 4 skills + 1 agent + 2 references | ~700 | Deterministic per-SHA | Low — read-only | **P1** | `ci-status.sh <head_sha_or_pr_number> [--format json\|summary]` → JSON `{total, passing, failing, in_progress, blocking: [...], in_progress_runs: [...]}` (summary mode prints one line). Exits: `0` CI clean and complete, `1` incomplete runs remain, `2` usage, `3` blocking failures present, `4` SHA/PR not found, `5` gh error. Distinct exit codes 1 vs 3 let callers branch on "wait" vs "fix". |
...
| **C-10** | Test-Plan checkbox extractor + updater (PR body) | `check-acceptance-criteria` Step 2 + 4, `merge` Step 3, `wrap` Step 2.2, `continue` Step 9, `subagent` Phase C, `phase-c-merger` Step 3 | 3 skills + 1 agent | ~500 | Deterministic text transformation | Low — the write call is `gh pr edit --body`, user-reviewable | **P1** | `ac-checkboxes.sh <pr_number> [--extract\|--tick <regex-or-indexes>\|--all-pass]` → `--extract` prints JSON `[{index, checked, text}]`; `--tick 0,2,3` or `--tick "regex"` updates the PR body via `gh pr edit`; `--all-pass` ticks every unchecked box. Exits: `0` OK, `1` no Test Plan section found, `2` usage, `3` PR not found, `4` gh edit failed. |
...
| **C-12** | Greptile daily budget check + atomic decrement | `phase-b-reviewer` "Daily Budget Check" (~35-line bash + 8-line jq block), `greptile.md` rule Step "Daily Budget", `cr-github-review.md` fallback paths | 1 agent + 2 rules | ~800 | Fully deterministic given today's ET date | Medium — write-back to session-state; must be atomic to prevent double-decrement | **P1** | `greptile-budget.sh [--check\|--consume\|--reset] [--budget N]` — `--check` prints JSON `{date, reviews_used, budget, exhausted: bool}`; `--consume` increments (with same-day reset) and prints the new state; `--reset` zeros today's counter. Exits: `0` consumed successfully, `1` exhausted (no decrement performed), `2` usage, `5` write failed. Used as guard before every `@greptileai` post. |
| **C-13** | pm-config section extractor (line-anchored `^## Header` parser) | `pm-okr` Step 2, `pm-update` Step 2, `prioritize` Step 0, `pr-review-help` Step 1a, `pm-handoff` Step 3, `pm-rate-team` Step 2, `pm-sprint-plan` Step 1, `pm-sprint-review` Step 2, `pm-team-standup` Step 2, `pm` Step 1A/1B.1 | 10 skills | ~600 | Deterministic — `awk`/`sed` line-anchor parse | Near-zero | **P1** | `pm-config-get.sh --section <name> [--json]` → stdout: raw section body (headings stripped) or `{}` if missing. Exits: `0` section present, `1` section missing, `2` config file missing, `3` usage. `--json` emits `{section: "<name>", content: "...", present: bool}` for structured callers. |
...
- **P0 — Extract `resolve-review-threads.sh`** — ref #271 C-04
- **P0 — Extract `repo-root.sh` root-worktree resolver** — ref #271 C-05
