...
| **C-19** ✅ | HHG two-ticket state-code extractor — **extracted in #292** | **Extracted to `.claude/scripts/hhg-state.sh`** (issue #292). Migrated: `wrap` Step 3.2 (inline `US_STATES` regex + two-pass grep replaced with a single script invocation; the surrounding HHG-match guard and empty-state warning are preserved). | 1 skill migrated | ~300 | Deterministic (regex against 50 USPS codes) | Near-zero | **P2** ✅ | `hhg-state.sh <text>` → stdout: matched USPS code (uppercased) or empty. Exits: `0` match, `1` no match, `2` usage. `--help` prints the header block. Pass 1 prefers HHG-adjacent codes (`TX HHG` / `HHG TX`); Pass 2 falls back to the first state code anywhere in the text. |
