...
| **C-14** ✅ | Reviewer-ownership detection (session-state first → review-history fallback → sticky flag) — **extracted in #287** | **Extracted to `.claude/scripts/reviewer-of.sh`** (issue #287). Migrated: `continue` Step 5 (inline session-state + 3-endpoint scan) and Step 6 rate-limit branch (legacy `"g"` sticky write → `--sticky greptile`); `phase-c-merger` init + Step 1 (handoff fallback now uses the shared resolver). `merge` Step 2 / `wrap` Step 2.1 / `status` Step 3 transitively migrated via `merge-gate.sh` (which already calls the same resolve_reviewer logic — kept in sync with this script). | 2 skills + 1 agent migrated directly; 3 skills via `merge-gate.sh` | ~350 | Deterministic given PR state | Low — read-only when default; write to session-state when `--sticky <reviewer>` | **P1** ✅ | `reviewer-of.sh <pr_number> [--sticky <cr\|bugbot\|greptile>]` → stdout: one of `cr`, `bugbot`, `greptile`, or `unknown`. Exits: `0` reviewer determined (printed), `1` cannot determine (`unknown` printed), `2` usage, `3` PR not found. `--sticky` writes the override atomically into session-state (preserving siblings) and prints the value. Legacy `"g"` entries are normalized to `"greptile"` on read. |
