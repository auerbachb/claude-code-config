...
| `/board` | The same table on demand, in any orchestration thread — the named command for the question `/subagent`'s on-demand answer handles in prose (issue #1581). **Partial in one row class:** queued membership is not durable, so a `/board` run from a thread that did not dispatch the round renders no queued rows and says so. Running and completed rows rebuild from durable state for their clocks, with merge state read live per PR (`gh pr view --json state,mergedAt`) — the one field the board does not take from disk |
