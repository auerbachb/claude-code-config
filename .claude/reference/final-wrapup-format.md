...
- **The two sit at different layers, so both firing is correct, not a contract breach.** `/wrap`'s "exactly one line, then nothing else" silent default (`wrap/SKILL.md` §Silent default) governs `/wrap`'s **own** output. The wrap-up is the **task's** terminal message — so a task that ends in a clean merge legitimately shows `merged PR #N` and then one wrap-up. A `/wrap` return is not itself the trigger: a fleet loop that wraps one merge-ready PR and keeps going has not reached terminal state, and emits only the merge line.
