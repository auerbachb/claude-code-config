...
`T_end = T_start + window_minutes * 60` is computed once in `/pause` Step 0 and is not re-evaluated. It is a budget on how long `/pause` waits on landing, not a promise to the user that work will finish.
...
**`window_expired: true`** is set in the pause block when the deadline passed before all `land` units finished. It tells resume to re-read GitHub rather than trusting the `landed` array as complete.
