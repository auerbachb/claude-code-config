...
| **Model the vendor's CodeRabbit cap in `cr_hourly` and pace against it** | The cap *is* hourly — **5 reviews/hour per developer** on Pro (#1204), the same model this record's role table and reconciliation item 2 use. *(Corrected 2026-09-01, #1517: this row previously described it as a "7-day rolling allowance", contradicting both.)* The option is still declined, on the counter's **scope** rather than the cap's shape: the App's meter is per **developer** and therefore spans every repo that developer touches, while `cr_hourly` counts only what this session did in this repo. (The local CLI is a *separate* allowance with its own cap — §Repo variance — so neither surface's counter can stand in for the other.) A local counter cannot predict a lockout it cannot see, and tightening our own pacing against a number we do not meter costs reviews without preventing one. `cr_hourly` is therefore retained as an explicitly-labelled *local pacing proxy, never CodeRabbit's meter* (`cr-github-review.md`); the authoritative signal is the `Review limit reached` banner's own retry window, which `escalate-review.sh` waits out. |
...
**Baseline updated:** `review-stack-baseline.json` CodeRabbit entry: `expected_caps: []` → `["rate_limit", "fair_usage"]`; `notes` rewritten with the measured window, both mechanisms with their PR citations, the 0/268 approval fact, and the App-versus-CLI conclusion. `role`, `gates_merge`, and `approves_via` unchanged. `measure.sh` now classifies CodeRabbit's Fair Usage refusal clause — `under our [Fair Usage Limits Policy]`, and its unlinked form — as a distinct `fair_usage` kind so the two mechanisms stay machine-readable. It anchors on the refusal clause rather than the bare policy name because CodeRabbit also *explains* the policy in prose and quotes this repo's own cap docs back at us; the bare phrase counted that as a cap (#1338).

## BugBot cap + chain-position reconciliation (#1304, 2026-08-26)

**Trigger:** D3 drift finding `bugbot/D3`, rated **high** because BugBot is the tier the chain falls through to. `review-stack-baseline.json` recorded `expected_caps: []` for BugBot — deliberately, so the first post-swap run would report its spend exhaustion as genuine drift (§Decision, BugBot row). It did. That run was **truncated** (60 PRs against a `--limit` of 60), so its counts were floors; this reconciliation rests on the same **non-truncated** measurement #1303 used — 2026-07-25 → 2026-08-26, **268 merged PRs**, `window.truncated: false`. Full report: [`review-stack-audit-2026-08.md`](./review-stack-audit-2026-08.md).

**Evidence — one mechanism, and it is exhausted:**
...
**The failure shape is not what the rules describe — measured correction.** `bugbot.md` and `pricing-matrix.md` both state that a spend-limited BugBot returns a `conclusion: "success"` check-run, making an exhausted budget look like a clean pass. In this window it does not: of **20 sampled capped PRs** spanning #982..#1332, 19 were retrievable (PR #1130 returns 404) and every one concluded **`neutral`** (17) or published no `Cursor Bugbot` check-run at all (2) — **zero `success`**. `success` is BugBot's *genuine* silent clean pass, observed on non-capped PRs [#963](https://github.com/auerbachb/claude-code-config/pull/963), [#965](https://github.com/auerbachb/claude-code-config/pull/965), and [#970](https://github.com/auerbachb/claude-code-config/pull/970). The dangerous conjunction therefore **did not occur** in the measured window. This makes the shape *less* dangerous than recorded, not more — and it is a sampled finding, not a census, so the guard stays.

**The guard holds either way, verified against real data.** PR #1265 is merged, so `merge-gate.sh` short-circuits on `PR #1265 is MERGED — not open` and never reaches the BugBot block; the verification therefore replays #1265's **real captured payload** (reviews, both comment endpoints, HEAD check-runs, real HEAD committer date `2026-08-23T01:45:21Z`) through the real `merge-gate.sh --reviewer bugbot`, with the CR-path approvers' review objects removed so the BugBot path is the only thing that could satisfy the gate:
...
| **B** | identical, conclusion mutated `neutral` → **`success`** (the exact AC conjunction) | `met: false` |
| **C** | *negative control:* `success`, refusal comments **stripped** | `met: true` |

C is what makes B meaningful: the only difference between them is the real refusal comment, and it flips the verdict. Two independent blocks are in play — `BB_HAS_FAILURE_COMMENT` short-circuits the silent-pass path, and `neutral` is not `success` anyway. Offline regression coverage passes unchanged (`merge-gate-bugbot.test.sh` 21/21, scenario 8 pins the `success`+refusal case).

**One-nudge-per-HEAD suppression holds.** `bugbot-refused-head.sh 1265 4cc2020…` exits **0** (suppress) on the real refused HEAD, against exit **1** (post) on PR #970's un-refused HEAD — the discriminating control. `maybe-trigger-bugbot-suppression.test.sh` passes 16/16. Budget is never burned re-nudging a reviewer that cannot answer until the cycle resets.

**Decision: BugBot keeps the first-fallback position. Record the cap; defer any demotion or cap change to its own reviewed PR.**

The issue framed "raise the cap" and "move it down the chain" as equally legitimate, to be settled by the number. The numbers decline both. **Moving it down** is the weaker option: below BugBot sits **Greptile**, and demotion promotes the *more* expensive reviewer into the first-fallback slot — Greptile is capped at $100 (#1228) against BugBot's $1,000, so the cheaper cap would be reached sooner and more often. BugBot was also **sole source of findings on 29 PRs** in the very window it spent capped, which is coverage a demotion erodes for no saving; a spend refusal is terminal for the cycle, not evidence the tool is low-value when it runs. **Raising the cap** costs roughly $400–500/month more (the run-rate needs a ceiling near $1,400–1,500) and remains the wrong first move: the free levers are untouched — Trigger Mode `Every Push` → per PR, Incremental Review → On, Review Draft PRs → Off, Effort `High` → Medium, and Autofix off (**299 runs, 0 merged**). Narrowing scope is strictly cheaper than buying headroom for reviews that are re-reviewing every push at high effort.

**The zero-approval figure is not a regression, and must not be read as one.** `gates_merge: true` and `approves_via: "check_run"` stay exactly as they are. BugBot signals a clean pass through the `Cursor Bugbot` check-run and posts no `APPROVED` review object, so recording it as a review-object approver would fire a permanent, false `D4` on every run — the precise failure the `gates_merge` / `approves_via` split exists to prevent.

**What this does not change:**

- No routing rule changed. `.claude/rules/bugbot.md` and `.claude/rules/cr-merge-gate.md` are untouched; this audit is advisory. The `neutral`-vs-`success` correction to `bugbot.md` §BugBot failure detection is a **follow-up**, not part of this PR.
- The chain order in §Decision is unchanged — CR → BugBot → Greptile → self-review.
- Related: **#1199** (the chain realignment this reconciles against), **#1303** (the CodeRabbit cap reconciliation from the same window), **#1204**/**#1228** (the dashboard and billing readings every cost figure rests on).

**Baseline updated:** `review-stack-baseline.json` BugBot entry: `expected_caps: []` → `["spend_limit"]`; `notes` rewritten with the measured window, the cap value and reset cadence with their dashboard citations, the 176/268 incidence, the 0/268 approval fact, and the `neutral`-not-`success` failure-shape correction. `role`, `gates_merge`, and `approves_via` unchanged. The `source` block now points at this record (previously #1287).
