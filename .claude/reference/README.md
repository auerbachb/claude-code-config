...
- `polling-backoff-warn-test-hotspot-decision.md` — diagnosis and KEEP decision for `polling-backoff-warn.test.sh` churn (3 merged PRs, Issue #1069); verdict: external-contract-driven churn tracking scheduling-substrate evolution (CronDelete → /loop → Monitor/TaskStop); 14 homogeneous backoff-ladder cases, no companion file, no cross-file helper duplication — keep as-is
