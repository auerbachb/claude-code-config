...
**The empty-value guard is now the rule, not the exception (issue #1577).** The same shape held on every flag whose value is read downstream through a `[ -n "$VAR" ]` test, and each fell back to a policy the caller never named: `--ref` to auto-detection (`origin/HEAD` -> `origin/main` -> `HEAD`), `--since` to the computed 14-day window, `--repo` to whatever `gh` infers from the cwd, and `--exclude` to the built-in list alone. All four now reject an empty value at parse time with exit 2 and a message naming the flag. The remaining value-taking flags need no guard and were left alone: `--threshold`, `--conflict-weight`, `--pr-cap`, `--top` and `--sweep-threshold` are already rejected with exit 2 by their numeric validators (whose `case` patterns match the empty string), `--source` by its enum validator. The invariant the block now holds: **every value-taking flag either rejects an empty value at parse time or is rejected downstream by its own validator** — none silently substitutes a default.
...
- Issue #1577 — the `--exemptions` empty-value guard generalised: `--ref`, `--since`, `--repo` and `--exclude` had the identical silent-fallback hole and now reject an empty value at parse time.
