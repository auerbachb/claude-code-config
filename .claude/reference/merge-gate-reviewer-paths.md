...
- **Freshness and fix-only reuse (issue #1000):** fresh signals still use the post-HEAD-push comment/review/inline path. When a later fix-only push has no fresh Greptile signal, the gate may reuse the latest completed review round's zero-P0 verdict. The durable round boundary is the latest exact `@greptileai` command posted by the PR author; prose mentions, bot text, and collaborator comments cannot create a boundary. Greptile evidence must follow the latest trusted trigger, including on the fresh path. An unanswered latest trigger fails closed. Legacy history without a retained trigger is scanned conservatively in full.
