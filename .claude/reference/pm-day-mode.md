...
**Weekly path.** Full `/stop` (default window and portable handoff). No auto-wake Monitor — a days-long sleep cannot be a persistent in-session Monitor, and the durable scheduler is declined for the reasons recorded in `cross-session-durability.md`. One-line notify naming `parked_until`. The user resumes manually with `/stop-resume --resume-refill`. **On session restart, a weekly stop is never auto-re-armed** — only `limit_kind="rolling_window"` triggers an in-session auto-wake; a readable non-rolling kind or an unreadable `limit_kind` both require manual resume.
