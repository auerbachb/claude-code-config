...
Both writes were unconditional before #1663 and predate the token; #1596 added `park_claim_token` to lists that were already clearing the record wholesale, so the field neither created nor widened the exposure. #1663 closed it by putting both sites on the one contract below.

### The park-retirement contract (#1663)

**Canonical for both cleanup sites** — `/pause-resume` Step 5's `retire_limit_park` and `/pm` D5's successful-resume clear. Each restates the mechanics in its own executable block (a skill cannot call the other's function); this is the definition those blocks implement, and neither may diverge from it silently.

**One read, bound.** Read `.repos["<key>"].day` **once**, with `--get-json`, and derive the anchor from that snapshot. Three separate `--get` calls read three different instants, so an anchor picked from one could name a record another never saw — the same one-read discipline the leave-time CAS in `/pause-resume` keeps for `.window`.

**Anchor precedence.** The clear is one `session-state.sh --cas <anchor>=null --expect <observed>` composed with the remaining field clears as `--set` flags, so the compare gates the whole batch (#1445) and a lost compare writes nothing at all:

| Observed | Anchor | Why |
|----------|--------|-----|
| `park_claim_token` non-null | the token | A 2D.7 claim is mid-assembly. The token is minted per claim, so it is the strongest identity in the record — stronger than the cause, which is a two-valued enum two different parks share. |
| else `limit_cause` non-null | `limit_cause` | A completed record. Coarser than the token by construction — see the residual below. |
| else | `parked_until` at the value read, **null included** | The no-identity record. It still clears — that is the #1595 manual-resume escape hatch, which a guard that refused a no-identity record would deadlock shut — and `parked_until` is the one field **every** claim shape writes (2D.6 and 2D.7 Step 1 alike), so the clear is refused only when the slot has actually changed hands, never on the record the step read. At `/pm` D5 that observed value is usually `null`, which is exactly the empty slot a fresh 2D.7 claim takes. |

**Exit contract.** `0` cleared · `7` **superseded** · `8` identity unreadable (fail closed, clear nothing) · anything else a write error. Lock timeout (`6`) retries once before being judged.

**What exit 7 obliges.** A newer park owns the slot and nothing was written, so its `parked_until`, `limit_kind`, `limit_cause`, token and wake identity are all left exactly as found. Stop nothing — a `TaskStop` aimed at the new owner's wake would leave a park with no wake at all, the stranding the single-slot design exists to prevent — retry nothing, and say so in one line. The resume is *resolved* from the retiring step's point of view: the new owner runs its own recovery.

**The residual, named.** The `limit_cause` anchor is a coarser identity than the token: two successive parks of the same kind both read `"reactive"`, so a clear pinned to that cause would win against a *replacement* park that had taken the slot through a full null-cause cycle in the window. Reaching it needs three ordered writes inside one read-to-write gap — cause cleared, then re-claimed — where the token and `parked_until` anchors need none. It is bounded by the two clear sites being the only writers that null a live cause, and it is the price of keeping a completed record guardable at all; the token tier covers the window that is actually reachable in practice.
