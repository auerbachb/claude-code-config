...
A capture thread cannot adopt, so its work leaves the thread — but it leaves **once, for the session**, not once per issue. One hand-off opens one execution-capable thread that bootstraps its own table and runs every filed issue inline.
...
A defect this shape introduces, and how it is handled: one `task_id` shared across issues means a single retract must **not** blindly withdraw the hand-off. `/issue-maker` Step 12 counts the other open issues sharing the id first — withdraw only when this was the last one, otherwise refresh the hand-off over the remaining set (replace, then withdraw).
