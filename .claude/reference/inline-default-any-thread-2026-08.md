...
3. **The census still counts per issue, and that is deliberate.** The hand-off's `task_id` is stamped on every issue it covers, because `active-work-cap.sh` and the cross-skill dedup both read the field per issue — without it, `/wave` or `/pm` would offer a second launch for covered work. So a session filing more issues than `FREE` can push `ACTIVE` past `CAP` and defer the *next* session's hand-off. That overshoot is bounded to one session and self-corrects, and it is the right side to err on: covering only `FREE` of the issues would strand the rest with no launch path, which is the outcome #1229 exists to prevent. Concurrency itself never overshoots — the launched thread still runs at most `min(pipeline_ceiling, active_work_cap)`.
...
**Known limitation — admission is checked, not reserved** ([#1238](https://github.com/auerbachb/claude-code-config/issues/1238)). `FREE > 0` gates the offer, but the census then records one unit per covered issue, so a batch larger than `FREE` is admitted and overshoots `CAP`; two concurrent sessions can likewise read the same free slot and both offer. Closing this properly means either reserving capacity atomically or teaching the census to count a hand-off as one unit and letting the launched thread queue — both are changes to `active-work-cap.sh`'s counting contract (#1191/#1224), not to this gate's prose, which is why they are tracked separately rather than bolted on here. The concurrent-sessions half is not new: two capture threads could always read the same `FREE`.
