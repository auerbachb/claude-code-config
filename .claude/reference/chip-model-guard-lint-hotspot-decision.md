...
| PR #1660 | #1398 | §1 mismatch-menu assertions | Added five checks for the AskUserQuestion mismatch branch — the menu vehicle, both family-level option labels (the switched-confirm one suffix-anchored on `(Recommended)`), the confirm-answer re-verification, and the headless-only prose fallback — each with a keyword-preserving inversion fixture. Added `extract_guard_block()` / `require_guard_pattern()`: preamble assertions match the fenced block, so prose cannot stand in for shipped bytes; plus exactly-one-block and exactly-two-numbered-options counts, and a free-text-escape assertion |

Issues #770, #791, #802, #837, and #1398 each added a new requirement that demanded a corresponding
