...
- `active_agents`: subagent records, a map keyed by agent id (issue #1631). Each value should include `id`, `task`, `issue`, `pr`, `phase`, `launched`, and optional `last_seen_at`. Write one key at a time (`--set '.active_agents["<id>"]=...'`) and delete with `--remove-agent <id>`; a whole-map replace loses a sibling thread's entries.
