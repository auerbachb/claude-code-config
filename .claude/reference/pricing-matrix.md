...
| **CLI** (`codeant review`) | Same Premium plan | **~10 agent reviews/day** — undocumented, observed only. One `review` invocation issues several `agent/turn` calls (5 files/turn), so one run is not one unit. 15-file cap per run. **Current state (2026-08-23): no credentials** — `~/.codeant/config.json` absent (`codeant scans orgs` returns "Missing API key"; review invocations return 403 on the review endpoint — see `local-review-cli-failure-modes.md` "CodeAnt: missing API key" for the 403 vs cap discrimination). Auth present through at least 2026-08-22 (PR #1265 PR body confirms daily cap). Restore with `codeant login` (browser OAuth) or `codeant set-codeant-api-key`. | **None published** | **No** | **No paid lever exists** | — | Separate from App — the App served 234 reviews on a day the CLI was locked out |
