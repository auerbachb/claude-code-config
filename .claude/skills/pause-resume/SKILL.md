  ...
  if [[ "$LIMIT_TASK_RC" -ne 0 && "$LIMIT_TASK_RC" -ne 3 ]]; then
    ...
    if TaskStop "$LIMIT_TASK_ID" 2>/dev/null; then
      if "$SESSION_STATE_SH" \
        ...
        --set ".repos[\"$REPO_KEY\"].day.limit_resume_generation=null"; then
