...
- **Default mode — pause only on a strong or exact match:** if the top candidate clears that bar, surface it and present the choice via **`AskUserQuestion`** when available (options: `"Skip — looks like a duplicate (Recommended)"`, `"File anyway"`; prose fallback `"Looks like a duplicate of #N — file anyway? (y/N)"` in headless runs). On a weak/ambiguous match, **do not block** — file, and name the near-duplicate as a decision point (Step 9a: "possibly overlaps #N"). No match → file normally. See `ask-menu.md` for the full vehicle convention.
...
**Every increment's own bound must be at or under `INCREMENT_BOUND_MIN`** (default 120 — resolved by the block in the top-level rule's sizing check, never typed in). Against the seed table **at that default** that means the **Light** row, `Est: 60–90 min · plan on 90`; the bound is what governs, not the row, so a knob set under 90 admits no seed row at all and the increment carries an explicit tighter estimate or is not cut yet. This binds chains from *either* trigger, not just the time one: a slice that needs the Standard row's 180 is a slice cut too coarse — re-cut the boundary rather than publishing an over-bound increment, and if it genuinely cannot be cut finer, say so in the decision points rather than letting the number slide.
...
**Cap: at most 5 increments per ask without pausing.** Five is a chain; twenty is a confetti backlog nobody will triage. If an ask genuinely needs more than 5, **stop before filing anything** and say so — name the count you'd need and what the increments would be, and present the choice via **`AskUserQuestion`** when available (options: `"Re-cut to ≤5 increments (Recommended)"`, `"File all N increments"`; prose fallback in headless runs). This mirrors the Step 4 dedup pause bar: clear the bar and proceed silently, exceed it and ask first. See `ask-menu.md` for the full vehicle convention.
...
- **Chip mode** (`mcp__ccd_session__spawn_task` present): **before calling `spawn_task`, register via the full contract** (see `chip-launching.md` "Offer Registry"): `REG_TID="$(chip-offer-registry.sh --emitter issue-maker --issue <primary-N> --cap-free "$FREE" --reserve 2>/dev/null)"` — exit 7 means defer (same as `FREE=0`); any other non-zero exit means proceed uncounted. **Retain `REG_TID`** separately from `chip_task_id` for later `--transition` calls (`running` on start, `pr-backed` when a PR opens, `done`/`retracted` on terminal events). Call `spawn_task` **once** with `title` (verb-first, ≤60 chars — e.g. `Run 5 captured issues (#1230 first)`), `prompt` (the block above, verbatim), `tldr` (1–2 plain sentences), `cwd` (repo root). On success, update `chip_task_id` on covered issues to the returned `task_id` (replacing the offer token), write `reg_tid` to `$LOG` at the top level (`set_log '.reg_tid = $v' --arg v "$REG_TID"`) so retraction can transition the registry entry if the offer is later withdrawn, and update `delivery_mode` to `"chip"` (`set_log '.delivery_mode = "chip"'`) — this is the unambiguous post-compaction marker that distinguishes a running chip task from a pending inline offer. Print only the short summary per `chip-launching.md` "Short-summary transcript format".
...
**Present the choice via `AskUserQuestion` when available** (options: `"Retract whole chain (Recommended)"`, `"Re-cut remainder"`; prose fallback in headless runs): it is the rare genuinely blocking call — both outcomes are destructive in different directions, and the increments are not interchangeable. State the successor numbers in the question text. In rapid-fire, ask this one anyway; it is the same class as the two hard bars. See `ask-menu.md` for the full vehicle convention.
...
- **`OFFER_TOKEN` present (offer pending) and `SHARERS == 0`** — this was the last issue the offer covered. Withdraw the offer entirely: clear `chip_task_id` to `null` and say so (*"Offer withdrawn — no remaining open issues."*). Use `delivery_mode` (read from `$LOG`) to determine cleanup: if `"inline"`, no chip dismiss is needed (the token is locally-generated, not a `spawn_task` ID); if `"chip"`, read `reg_tid` from `$LOG` and transition it: `chip-offer-registry.sh --transition --task-id "$REG_TID" --state retracted` — then clear both `chip_task_id` and `reg_tid` to `null` and `delivery_mode` to `null` in the log.
