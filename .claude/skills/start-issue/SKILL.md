...
- `SPLIT_THRESHOLDS` empty → **optional**. Print `DEGRADED: split-thresholds.sh not found (checked all three paths) — using default split threshold 180` and read Step 7's time trigger against that default. The threshold is a knob, so an unresolved helper must be *said*, not silently defaulted — a lowered `SPLIT_OVER_MIN` that never loaded is a split that never fires.
