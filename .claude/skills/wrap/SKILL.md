...
3. **Exit `1` — classify JSON** (never guess from prose alone):
   ...
   **Stop conditions after `/fixpr`:**
   ...
   - **`BEHIND`** → `/fixpr` rebased/force-pushed; re-run the gate. If CI is now pending on the new HEAD, treat as `CI_PENDING`. If instead `/fixpr` reported a clean `BEHIND` auto-merged via `admin-merge.sh --auto-plain` (issue #754), the PR is **already merged** — skip the rest of the loop, relay its `AUTO_PLAIN_MERGED` evidence block, and go to the post-merge phases.
...
- **Never** call GitHub APIs that modify **branch protection** (`.../branches/.../protection`). When a merge is blocked by `enforce_admins` on a solo-owner repo, suggest **`/admin-merge`** (Branch E) — it prints a command for the **user** to run — instead of toggling protection or pointing the user at the GitHub UI. Unaffected by this: `admin-merge.sh --auto-plain`, which issues **no** protection call at all and therefore auto-runs (#754).
...
- **Never auto-act on anything affecting shared state** — live `CronCreate` jobs (session-scoped; `durable: true` is a no-op), active subagents, human-owned issues/PRs, recovery branches. Surface them; let the user decide.
...
- **Surface (never auto-act).** Add to `SWEEP_NEEDS_DECISION`: live `CronCreate` jobs still scheduled in this session (`CronList`), any `active_agents` entries (running subagents), monitor-mode flags (`monitoring_active=true`), and any `recovery/dirty-main-*` branches left by `dirty-main-guard.sh`. Note: `CronCreate` jobs are session-scoped and will not appear in a future session regardless of the `durable` flag. Word each as e.g. `Active subagent still running: PR #620 Phase C — stop it or let it finish?` or `Cron job <id> still scheduled this session (<prompt>) — keep or CronDelete?`.
