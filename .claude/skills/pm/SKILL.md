...
- `MAKESPAN_SH` / `ESTIMATE_RESOLVE_SH` empty → **optional** (needed only for window batch fitting and estimate display). If unavailable when a window is active, skip window-fit trimming with a `DEGRADED:` note; show estimates inline where available.
