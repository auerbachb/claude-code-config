...
> **The checkout class is report-only and exit-code neutral.** It is never removed by plain `--apply` (that needs `--remove-orphaned-checkouts`, which this step does not pass, because it deletes working-tree files), and it deliberately does not move the exit code — so a run whose only finding is orphaned checkouts exits **0**. Read `orphaned_checkouts[]` and the `checkout_scan` scalar (`"ok"` / `"none"` / `"unreadable"`) regardless of `RC`, and surface them; `"unreadable"` means nothing was classified, so report the sweep as incomplete rather than clean.
