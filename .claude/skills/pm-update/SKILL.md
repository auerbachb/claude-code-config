...
`stale-cleanup.sh` reports five categories — stale worktrees, stale local branches, stale remote branches, orphaned worktree registrations (`.git/worktrees/<id>` entries with no live worktree behind them; issue #1402), and orphaned worktree *checkouts* (directories under `.claude/worktrees/` whose registration is gone; issue #1417) — plus a "Skipped (safety)" list with the reason each item was protected (main worktree, caller's current worktree, uncommitted changes, open PR, protected branch name, branch checked out in a worktree, locked or caller-owned registration). See `stale-cleanup.sh --help` for the full safety-check contract.

> **The checkout class is report-only and exit-code neutral.** It is never removed by plain `--apply` (that needs `--remove-orphaned-checkouts`, which this step does not pass, because it deletes working-tree files), and it deliberately does not move the exit code — so a run whose only finding is orphaned checkouts exits **0**. This step runs `--check` **without** `--json`, so read the class from the **text report** regardless of `RC` — the `Orphaned worktree checkouts (report-only — …)` section and its `<path> — <reason>` lines, plus any checkout rows under `Skipped (safety):`. Never let `RC == 0` short-circuit Step 8.2 while that section is non-empty. A `WARNING: the orphaned-checkout scan directory could not be read` line means nothing was classified, so report the sweep as incomplete rather than clean — `Orphaned worktree checkouts: none` under that warning is not a clean bill of health. (The structured equivalents, `orphaned_checkouts[]` and the `checkout_scan` scalar, exist only under `--json`, which `/pm-clean` Step 2.1 captures.)
