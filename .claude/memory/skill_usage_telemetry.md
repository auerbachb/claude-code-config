...
**Exactly-once dedupe:** the detector records, then drops a marker at `~/.claude/.skill-usage-pending/<session>__<skill>` holding the epoch it fired. A `Skill` call for the same session + skill consumes a fresh (≤300s) marker and skips its own line. Consume-once plus the freshness bound cap the failure modes at one line, and keep this correct if a future build starts emitting `Skill` calls for pre-expanded commands.
